...
export default async function NovelPage({
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <SearchBox kind="novel" source={source} placeholder={`Search ${source}...`} />
      ...
      <NovelContent source={source} />
    </div>
  );
}

async function NovelContent({ source }: { source: string }) {
  let items: unknown[] = [];
  ...
  return <SourceGrid items={items as never[]} empty={`No home items from ${source}.`} source={source} kind="novel" />;
}
